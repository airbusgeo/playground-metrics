...
def compute_statistics_given_rectangle_matches(groundtruth_rects_matched, rects_matched):
  ...
  return {'groundtruth_rects_matched': groundtruth_rects_matched,
...
def compute_average_precision_recall_given_precision_recall_dict(
  ...
  Args:
      ...
      calcualted under a range of iou_thresholds, where the iou_threshold is
      the key.
  Returns:
...
def compute_average_precision_recall(
    groundtruth_coordinates, coordinates, iou_threshold):
  """Computes the average precision (AP) and average recall (AR).
  ...
  if (len(groundtruth_coordinates) % 4 != 0) or (len(coordinates) % 4 != 0):
    raise ValueError('groundtruth_info_dict and test_info_dict should hold '
                     'only 4 * N numbers.')
