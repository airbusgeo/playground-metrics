...
def score(iou_threshold, gt_coords, gt_chips, gt_classes, pchips, boxes_dict):
    ...
    vals = {'map': np.nanmean(average_precision_per_class),
            ...
            'map_score': np.nanmean(per_class_p),
            'mar_score': np.nanmean(per_class_r)}
    return vals
    #
...
def score_from_files(path_predictions, path_groundtruth, iou_threshold=.5):
    """
    Compute metrics on a number of prediction files, given a folder of prediction files
    and a ground truth.  Primary metric is mean average precision (mAP).
